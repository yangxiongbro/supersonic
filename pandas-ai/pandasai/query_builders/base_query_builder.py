...
class BaseQueryBuilder:
    ...
    def build_query(self) -> str:
        # if self.schema.source.type=='parquet':#本地文件
        #     table_name=self.schema.name
        # else:
        #     if self.schema.source.type=='trino':
        #         table_name=self.schema.source.connection.database+"."+self.schema.source.table
        #     else:
        ...
        if self.schema.source.type=='trino':
            ...
            table_name=self._get_table_expression()
        query = select(*self._get_columns()).from_(table_name)
    ...
    def get_head_query(self, n=5):
        if self.schema.source.type=='trino':
            # table_name=self.schema.source.connection.database+'.'+self._get_table_expression()
            if self.schema.source.view is not None:
                table_name='('+self.schema.source.view+')'
            else:
                table_name=self.schema.source.connection.database+'.'+self.schema.source.table
    ...
    def get_row_count(self):
        if self.schema.source.type=='trino':
            ...
            table_name=self._get_table_expression()          
        return select("COUNT(*)").from_(table_name).sql(pretty=True)
