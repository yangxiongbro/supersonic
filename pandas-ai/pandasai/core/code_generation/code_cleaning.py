...
class CodeCleaner:
    ...
    def _replace_table_names(
        ...
        for table_name in table_names:
            if table_name in allowed_table_names:
                ...
                sql_query = regex_patterns[table_name].sub(table_name, sql_query)
            else:
    ...
    def _clean_sql_query(self, sql_query: str) -> str:
        ...
        # # allowed_table_names = {
        # #     df.schema.name: df.schema.name if df.schema.source.type=='parquet' else df.schema.source.table for df in self.context.dfs
        # # } | {f'"{df.schema.name}"': f'"{df.schema.name}"' if df.schema.source.type=='parquet' else f'"{df.schema.source.table}"'  for df in self.context.dfs}
        
        # allowed_table_names=[]
        # for df in self.context.dfs:
        #     allowed_table_names.append(df.schema.name if df.schema.source.type=='parquet' else df.schema.source.table)
        #     allowed_table_names.append(f'"{df.schema.name}"' if df.schema.source.type=='parquet' else f'"{df.schema.source.table}"')
