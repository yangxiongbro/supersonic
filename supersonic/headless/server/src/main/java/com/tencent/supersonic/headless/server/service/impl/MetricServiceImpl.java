...
public class MetricServiceImpl extends ServiceImpl<MetricDOMapper, MetricDO>
    ...
    @Override
    public void deleteMetricByModels(List<Long> moduleIdList, User user) {
        MetricsFilter filter = new MetricsFilter();
        filter.setModelIds(moduleIdList);
        List<MetricDO> metricDOList = metricRepository.getMetrics(filter);
        if(CollectionUtils.isEmpty(metricDOList)){
            return;
        }
        for(MetricDO metricDO : metricDOList){
            metricDO.setStatus(StatusEnum.DELETED.getCode());
            metricDO.setUpdatedAt(new Date());
            metricDO.setUpdatedBy(user.getName());
        }
        metricRepository.batchUpdate(metricDOList);
    }
